		...
		<div class="box_body test-1">
			...
			<div class="rele_box test-1">
				...
				<Form :model="formItem" :label-width="180" @submit.native.prevent>
					...
					<FormItem label="开始日期" class='formItem'>
						<DatePicker type="datetime" placeholder="请依次写出年月日时"   v-model="itemlist.add_date" v-if="itemlist != undefined"></DatePicker>
						<DatePicker type="datetime" placeholder="请依次写出年月日时"   v-model="datesA" v-else></DatePicker>
					...
					<FormItem label="完成日期" class='formItem'>
						...
						<DatePicker type="datetime" placeholder="请依次写出年月日时" v-model="datesB" v-else></DatePicker>
					</FormItem>
	...
	export default {
		...
		methods: {
			...
			asiospust() {
				...
				if(that.itemlist) {
					dateA = that.itemlist.add_date
					dateB = that.itemlist.plan_date_limit
				} else {
					dateA = that.datesA;
					dateB = that.datesB;
				}
				let datasA = new Date(dateA);
				let datasB = new Date(dateB);
				let date_valueA = datasA.getFullYear() + '-' + (datasA.getMonth() + 1) + '-' + datasA.getDate() + ' ' + datasA.getHours() + ':' + datasA.getMinutes() + ':' + datasA.getSeconds();
				let date_valueB = datasB.getFullYear() + '-' + (datasB.getMonth() + 1) + '-' + datasB.getDate() + ' ' + datasB.getHours() + ':' + datasB.getMinutes() + ':' + datasB.getSeconds();
				console.log(date_valueA,date_valueB);
				//计划id不为空就更新
				if(that.itemlist != undefined) {
					if(that.itemlist.plan_date_limit == '') {
						...
						lis.plan_start_time = date_valueA
						lis.plan_time_limit = date_valueB
						if(that.onshows) {
...
//										that.onshows = false
					...
					if(that.formItem.dates == '') {
						...
						liform.plan_id = that.$store.state.plan_id
						liform.plan_name = that.formItem.title
						liform.plan_desc = that.formItem.textarea
						liform.plan_belonged = that.task_belonged
						liform.plan_start_time = date_valueA
						liform.plan_time_limit = date_valueB
						liform.structure_id=that.$route.query.structureId
