			...
			<div class="com_box" v-for="(item,index) in myplan">
				<div class="ul_div">
					<div @click.stop="o_iss(item,index)" class="li_div" :class="{li_div_finish : item.progress == 100 }">
						...
						<div style="color:#9158F5; margin-left:10px; display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp:1;overflow: hidden;">
							<!--{{item.real_name}}-->
							<span style="margin:0 5px;" v-for="(dataitem,index) in  item.plan_belonged_data">{{dataitem.real_name}}</span>
						</div>
					...
					<transition name="slide-fade">
						<div class="list_box" v-show="isshow == index">
							...
							<div class="listTitle">
								...
								<span>起止时间 ：{{item.add_date}}&nbsp; - &nbsp;{{item.plan_date_limit}}</span>
							</div>
							<div class="listPart">
											...
											<div class="proContent">
												<div class="progress_box">
													<div style="width:100%; text-align: center;">
														<i-Circle dashboard :percent="itlist.progress" stroke-linecap="round" stroke-color="#43a3fb">
								...
								<div class="processRight">
									...
									<div class="journalContainer">
														...
														<div class="smallReaply">
															...
															<div class="smallEvaBox">
																...
																<div class="typeProgress">
																	<Slider inputSize="default" v-model.sync="typePro" :tip-format="format"></Slider>
										...
										<div class="journalBox">
											...
											<div class="typeProgress">
												<Slider v-model.sync="typePro" :tip-format="format" inputSize="default"></Slider>
												<span style="margin-left:20px;">{{typePro}}%</span>
											</div>
				...
				that.axios({
					...
					.then(function(val) {
						if(val.data.error == 0) {
							...
							var plan = that.myplan
//							plan.forEach(item=>{
//								console.log(item.plan_sub_id)
//							})
//							for( var i=0; i<plan.length;i++ ){
//								console.log(plan[i].plan_sub_id);
//							}
							/* 获取初始化百分比数值  */
			...
			o_iss(item, index) {
				...
				that.monShow = false;
				that.perShow = false;
				that.typePro = Number(item.plan_process[0].progress)
					...
					.then(function(res) {
						if(res.data.error == 0) {
							...
							for(let i = 0; i < res.data.data.list.length; i++) {
								...
								console.log(that.progresList, "aas")
							}
						} else {
...
<style type="text/css">
	...
	.my_kask_x .smallEvaBox textarea.ivu-input,
	.my_kask_x .journalBox textarea.ivu-input {
	...
	.com_box .slide-fade-enter-active {
	  transition: all 1s ease;
	}
	.com_box .slide-fade-leave-active {
	  transition: all .8s cubic-bezier(1.0, 0.5, 0.8, 5.0);
	}
	.com_box .slide-fade-enter, .slide-fade-leave-to
